...
class ToolBase:
    ...
    and the agent it's bound to (for access to the chat state), as well as common setup/teardown hooks.
    """

    def __init__(self, app: "AutoAgentSystem", agent: "RecursiveAgent"):
        self.app: "AutoAgentSystem" = app
        """The app session this tool is running in."""
        self.agent: "RecursiveAgent" = agent
        """The agent this tool is bound to."""

    async def setup(self):
        """Called once per bound instance in an async context for each time this tool is bound to a new agent.
    ...
    async def cleanup(self):
        """Called each time the agent this tool is bound to completes its task.

        Override this method to clean up any ephemeral chat round-scoped resources if needed, but maintain any other
        state as the bound agent might still run again.
        """
        pass
